...
| Data retention | The fine-tuning dataset is archived in a Contoso-controlled storage account (the same Key Vault and managed-identity posture as the rest of the solution). Microsoft does not retain customer fine-tuning data after the job completes, per the [Azure OpenAI data privacy documentation](https://learn.microsoft.com/azure/ai-services/openai/concepts/data-privacy). |
