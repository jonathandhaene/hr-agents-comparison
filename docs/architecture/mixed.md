...
- **UC4 (mobility) and UC5-summary use a Foundry connected agent.** A small `ContosoHRMobilityAdvisor` agent (Microsoft Agent Framework) has only two tools (`match_internal_jobs`, `summarize_feedback_raw`) and is published as a REST endpoint; Copilot Studio invokes it via the Foundry connector.
...
| 4 Mobility | Topic → Foundry connector → `ContosoHRMobilityAdvisor` (tool: `match_internal_jobs` → HR API) |
| 5 Feedback (open) | Topic → `HR_FeedbackOpen` flow → HR API + Outlook + Dataverse |
| 5 Feedback (summary) | Topic → Foundry connector → `ContosoHRMobilityAdvisor` (tool: `summarize_feedback_raw`) |
