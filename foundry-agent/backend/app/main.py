"""Foundry-agent HR API — FastAPI backend for Solution C.

A lightweight REST API backed by the shared fixtures. Implements all endpoints
called by the Foundry agent tools (UC1–UC7).

Notes:
- Auth: none for local dev; in production pin behind APIM with managed identity.
- State is in-memory (resets on restart). For production, persist mutable state
  to Cosmos DB or Azure SQL.
- UC7 narrative drafting calls the fine-tuned Foundry model when
  FOUNDRY_NARRATIVE_DEPLOYMENT is set; otherwise returns a deterministic demo
  draft so the demo runs without a checkpoint.
"""
...
# Mutable state — replace with Cosmos in production.
...
# Policies — markdown files in the policies subfolder.
...
def _draft_narrative(employee: dict, grade: str, cycle_id: str, manager_notes: str, feedback_summary: str) -> str:
    ...
    Production path: call the fine-tuned Foundry model deployment
    (FOUNDRY_NARRATIVE_DEPLOYMENT env var).
    Demo / offline path: use the approved fixture narratives as a template.
    """
    ...
    if deployment:
        try:
            ...
            pass  # fall through to demo draft
...
# ---------------------------------------------------------------------------
# Routes — Policies (UC1 fallback for local dev)
# ---------------------------------------------------------------------------
...
def list_policies() -> list:
    if not _POLICIES_DIR.exists():
        return []
    return [{"name": p.stem, "file": p.name} for p in sorted(_POLICIES_DIR.glob("*.md"))]
