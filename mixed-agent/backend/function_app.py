...
def _draft_narrative_text(employee: dict, grade: str, cycle_id: str, manager_notes: str, feedback_summary: str) -> str:
    """Return a grade-calibrated draft narrative.

    Production path: call the fine-tuned Foundry model via the Foundry connected
    agent (the `/api/agent/invoke` route). Demo / offline path: use the approved
    fixture narratives as a template so the whole stack runs without a fine-tuned
    checkpoint.
    """
    # In the mixed-agent pattern the fine-tuned model is invoked through the
    # Foundry connected agent (draft_performance_narrative tool). The Functions
    # backend only stores and serves the draft; the generation itself happens in
    # the Foundry agent. Here we return a deterministic demo draft.
...
# ---------------------------------------------------------------------------
#
# In the mixed-agent the narrative text is generated by the Foundry connected
# agent (draft_performance_narrative tool), which calls the fine-tuned model
# deployment. The Functions backend stores the draft and the final submission.
# The `/api/narratives/draft` route is also used by the Foundry agent's
# `draft_performance_narrative` tool when it proxies the generation through the
# backend (offline demo path).


@app.route(route="narratives/draft", methods=["POST"])
def narratives_draft(req: func.HttpRequest) -> func.HttpResponse:
    try:
        body = req.get_json() or {}
    except ValueError:
        return _problem(400, "Invalid JSON body")
    ...
    if not (employee_id and cycle_id):
        return _problem(400, "employeeId and cycleId are required")
    emp = _emp(employee_id)
    if not emp:
        return _problem(404, "Employee not found")
    ...
    draft_text = _draft_narrative_text(emp, grade, cycle_id, manager_notes, feedback_summary)
...
@app.route(route="narratives/submit", methods=["POST"])
def narratives_submit(req: func.HttpRequest) -> func.HttpResponse:
    try:
        body = req.get_json() or {}
    except ValueError:
        return _problem(400, "Invalid JSON body")
    ...
    if not (draft_id and approved_text):
        return _problem(400, "draftId and approvedText are required")
    draft = _NARRATIVE_DRAFTS.get(draft_id)
    if not draft:
        return _problem(404, "Draft not found")
    if draft.get("status") == "submitted":
        return _problem(409, "Draft already submitted")
